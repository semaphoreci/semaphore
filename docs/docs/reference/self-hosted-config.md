...
This is the endpoint the agent connect to to register, sync, and pull job information. This is your the same as your [organization URL](./organizations#general-settings).
...
Agents generate a random name when this argument is empty. Set this value to force a specific name for this agent. Name length must be between 8 and 64 characters.
...
Command line argument `--env-vars` expect a comma-separated lists of `VAR=VALUE`. For example:
...
- `always`: job los are always uploaded
...
A path to a Bash or PowerShell script to be executed before the agent starts a job. This settings allows you to set up the environment before the job takes place.
...
A Bash or PowerShell script to be executed just before the agent shuts down. This can be useful to run clean up operations like pushing the agents logs to an external storage. 
...
T
During shutdown, the reason for shutdown is specified in the environment variable `SEMAPHORE_AGENT_SHUDOWN_REASON`. Possible values are:
...
The agent stops the job and shutdowns immediately after receiving an interruption request. Set this value to number greater than 0 seconds to force the agent to wait this amount of time before shutting down.
